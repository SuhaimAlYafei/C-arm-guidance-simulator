...
def main():
    ap=argparse.ArgumentParser(); ap.add_argument("--data-root",type=Path,required=True); ap.add_argument("--epochs",type=int,default=30); ap.add_argument("--batch-size",type=int,default=16); ap.add_argument("--workers",type=int,default=8); ap.add_argument("--image-size",type=int,default=320); ap.add_argument("--lr",type=float,default=1e-4); ap.add_argument("--output",type=Path,default=Path("results/mura_anatomy_resnet50_320")); ap.add_argument("--early-stop-patience",type=int,default=8); args=ap.parse_args()
    ...
    print(f"Images: train={len(tr)} valid={len(va)} size={args.image_size} batch={args.batch_size}")
    ttr=transforms.Compose([transforms.Resize((args.image_size,args.image_size)),transforms.RandomRotation(5),transforms.RandomAffine(0,translate=(.03,.03),scale=(.95,1.05)),transforms.ColorJitter(.08,.08),transforms.ToTensor(),transforms.Normalize([.485,.456,.406],[.229,.224,.225])]); tva=transforms.Compose([transforms.Resize((args.image_size,args.image_size)),transforms.ToTensor(),transforms.Normalize([.485,.456,.406],[.229,.224,.225])])
    ...
    train=dl(DS(tr,root,ttr),True); valid=dl(DS(va,root,tva),False)
    device=torch.device("cuda" if torch.cuda.is_available() else "cpu"); print("Device:",device,torch.cuda.get_device_name(0) if device.type=="cuda" else "")
    if device.type!="cuda": raise SystemExit("CUDA required")
    model=models.resnet50(weights=models.ResNet50_Weights.DEFAULT); model.fc=nn.Linear(model.fc.in_features,len(CLASSES)); model=model.to(device,memory_format=torch.channels_last)
    loss_fn=nn.CrossEntropyLoss(); opt=torch.optim.AdamW(model.parameters(),lr=args.lr,weight_decay=1e-4); sched=torch.optim.lr_scheduler.ReduceLROnPlateau(opt,mode="max",factor=.5,patience=2,min_lr=1e-6); scaler=torch.amp.GradScaler("cuda")
    args.output.mkdir(parents=True,exist_ok=True); history=[]; best=-1.; bad=0; best_epoch=0
    for e in range(1,args.epochs+1):
        ...
        rec={"epoch":e,"seconds":round(time.time()-t,2),"lr":opt.param_groups[0]["lr"],"train":tm,"valid":vm}; history.append(rec); (args.output/"history.json").write_text(json.dumps(history,indent=2),encoding="utf-8")
        ck={"epoch":e,"classes":CLASSES,"model":model.state_dict(),"optimizer":opt.state_dict(),"metrics":vm,"architecture":"resnet50","image_size":args.image_size}; torch.save(ck,args.output/"last_checkpoint.pt")
        if improved: torch.save(ck,args.output/"best_checkpoint.pt"); (args.output/"best_metrics.json").write_text(json.dumps(rec,indent=2),encoding="utf-8")
        ...
        if bad>=args.early_stop_patience: print(f"Early stopping after {bad} epochs without improvement."); break
