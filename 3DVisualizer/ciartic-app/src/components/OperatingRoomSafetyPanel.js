...
export default function OperatingRoomSafetyPanel() {
  ...
  return h(
    ...
    h('div', { style: { display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '5px', marginTop: '8px' } },
      h('button', { type: 'button', onClick: toggleOperatingRoomEnvironment, style: { ...baseButton, padding: '7px 5px', background: snapshot.environmentVisible ? '#0f766e' : '#334155' }, title: 'Toggle IV pole, staff, anesthesia station, carts and surgical light (O)' }, snapshot.environmentVisible ? 'OR ON' : 'OR OFF'),
      h('button', { type: 'button', onClick: toggleSafetyBubbles, style: { ...baseButton, padding: '7px 5px', background: snapshot.bubblesVisible ? '#7c3aed' : '#334155' }, title: 'Toggle collision/safety envelopes (B)' }, snapshot.bubblesVisible ? 'BUBBLE ON' : 'BUBBLES'),
      h('button', { type: 'button', onClick: toggleOperatingRoomEditMode, disabled: !interaction.ready, style: { ...baseButton, padding: '7px 5px', background: interaction.editMode ? '#d97706' : '#334155', opacity: interaction.ready ? 1 : 0.55, cursor: interaction.ready ? 'pointer' : 'not-allowed' }, title: 'Edit OR layout: select and drag equipment on the floor (E)' }, interaction.editMode ? 'EDITING' : 'EDIT OR'),
    ),
    interaction.editMode
      ? h('div', { style: { marginTop: '6px', padding: '7px', borderRadius: '6px', border: '1px solid rgba(251,191,36,0.35)', background: 'rgba(120,53,15,0.14)', color: '#fde68a', fontSize: '8px', lineHeight: 1.4 } }, interaction.selectedLabel ? `Selected: ${interaction.selectedLabel}. Drag it across the floor.` : 'Click an IV pole, Mayo stand, trolley, anesthesia machine, monitor cart, surgeon, or scrub nurse, then drag it.')
      : null,
    h('button', { type: 'button', onClick: resetOperatingRoomLayout, disabled: !interaction.ready, style: { ...baseButton, width: '100%', marginTop: '6px', background: '#1e293b', opacity: interaction.ready ? 1 : 0.55, cursor: interaction.ready ? 'pointer' : 'not-allowed' }, title: 'Restore the original OR equipment positions' }, 'RESET OR LAYOUT'),
    interaction.message ? h('div', { style: { marginTop: '6px', color: '#cbd5e1', fontSize: '7.5px', lineHeight: 1.35 } }, interaction.message) : null,
    h('div', { style: { marginTop: '7px', color: '#94a3b8', fontSize: '7.5px', lineHeight: 1.35 } }, 'E = edit layout · B = bubbles · O = OR equipment. Moving an object invalidates the old route; MOVE C-ARM is guarded until PREVIEW PATH checks the new layout.'),
  );
}
