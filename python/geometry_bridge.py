...
def build_diffdrr_pose(
    ...
    isocenter = isocenter_delta_sim.copy()
    ...
    # Diagnostic: compare the frontend-exported camera rotation with the
    # final DiffDRR camera rotation after axis-convention conversion.
    print()
    print("==============================")
    print("EXPORTED camera_to_ct rotation")
    print("==============================")
    print(camera_to_ct[:3, :3])

    print()
    print("==============================")
    print("POSE rotation sent to DiffDRR")
    print("==============================")
    print(pose_matrix[:3, :3])

    # Convert the exported frontend basis into DiffDRR's camera-axis
    # convention before comparing the matrices.
    exported_detector_x = transform_direction(
        SIM_CT_TO_DIFFDRR,
        camera_to_ct[:3, 0],
        "diagnostic exported detector X",
    )
    exported_beam = transform_direction(
        SIM_CT_TO_DIFFDRR,
        camera_to_ct[:3, 2],
        "diagnostic exported beam",
    )

    exported_detector_x = exported_detector_x - exported_beam * float(
        np.dot(exported_detector_x, exported_beam)
    )
    exported_detector_x = normalize_vector(
        exported_detector_x,
        "diagnostic orthogonalized exported detector X",
    )
    exported_detector_y = normalize_vector(
        np.cross(exported_beam, exported_detector_x),
        "diagnostic reconstructed exported detector Y",
    )

    exported_detector_x, exported_detector_y, diagnostic_roll_flip = (
        canonicalize_detector_roll(
            exported_detector_x,
            exported_detector_y,
            exported_beam,
    ...
    exported_rotation_diffdrr = np.column_stack(
        (
            exported_detector_x,
            -exported_beam,
            exported_detector_y,
    ...
    validate_rotation(
        exported_rotation_diffdrr,
        "diagnostic exported DiffDRR rotation",
    )

    rotation_difference = (
        exported_rotation_diffdrr.T @ pose_matrix[:3, :3]
    )
    validate_rotation(rotation_difference, "diagnostic rotation difference")
    difference_angle = rotation_angle_degrees(rotation_difference)

    print()
    print("==============================")
    print("EXPORTED rotation converted to DiffDRR convention")
    print("==============================")
    print(exported_rotation_diffdrr)
    ...
    print()
    print("Diagnostic 180-degree roll correction applied:", diagnostic_roll_flip)
    print(f"Difference angle = {difference_angle:.6f} degrees")
