...
def unpack_batch(
    ...
    Supported tuple layouts include:
        (image, label, target_position, current_position)
        (image, label, target_position, current_position, filename)

    Supported dictionary keys include common naming variants.
    """
    ...
    if isinstance(batch, dict):
        ...
        label = next(
            (
                batch[k]
                for k in ("label", "labels", "class", "target_class")
                if k in batch
            ),
            None,
        )
        ...
        metadata = {k: v for k, v in batch.items() if k not in consumed}

    elif isinstance(batch, (list, tuple)):
        if len(batch) < 4:
            raise ValueError(
                "The model requires current_position, but the dataset returned "
                f"only {len(batch)} values. Expected at least:\n"
                "(image, label, target_position, current_position)"
            )

        image = batch[0]
        label = batch[1]
        target_coords = batch[2]
        current_position = batch[3]

        if len(batch) > 4:
            metadata["extra"] = batch[4:]

    else:
        raise TypeError(f"Unsupported batch type: {type(batch).__name__}")

    if image is None:
        raise ValueError("Could not find the image tensor in the dataset batch.")
    if label is None:
        raise ValueError("Could not find the class label in the dataset batch.")
    if target_coords is None:
        raise ValueError("Could not find the target position in the dataset batch.")
    if current_position is None:
        raise ValueError(
            "Could not find current_position in the dataset batch. "
            "Do not substitute the target position because that would leak the answer."
        )

    return image, label, target_coords, current_position, metadata
...
with torch.inference_mode():
    for index, batch in enumerate(TEST_LOADER):
        images, labels, target_coords, current_position, metadata = unpack_batch(batch)
        ...
        current_position = current_position.to(
            DEVICE,
            non_blocking=True,
        ).float()

        output = MODEL(images, current_position)
